...
#
# PURPOSE:      Applies a resampling method to each map in a space time
#               raster dataset.
...
#%module
#% description: Applies a resampling method to each map in a space time raster dataset.
#% keyword: temporal
#% keyword: resample
#% keyword: raster
#% keyword: time
...
#% key: method
#% type: string
#% description: Resample operation to be performed on the raster maps
#% required: yes
#% multiple: no
#% options: nearest,bilinear,bicubic,lanczos,average,median,mode,minimum,maximum,quart1,quart3
#% answer: nearest
#%end
...
def main():
    ...
    new_sp = tgis.check_new_stds(output, "strds", dbif=dbif,
                                               overwrite=overwrite)
    # Configure the resampling module
    if method in ("nearest", "bilinear", "bicubic", "lanczos"):
        resample_module = pymod.Module("r.resamp.interp", input="dummy",
                                       output="dummy", run_=False,
                                       finish_=False,
                                       method=method, overwrite=overwrite,
                                       quiet=True)
    else:
        resample_module = pymod.Module("r.resamp.stats", input="dummy",
                                       output="dummy", run_=False,
                                       finish_=False,
                                       method=method, overwrite=overwrite,
                                       quiet=True)
    ...
    # run r.resamp.* all selected maps
    for map in maps:
        count += 1
